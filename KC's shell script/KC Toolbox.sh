...
load_functions() {
    ...
    if curl -sL --connect-timeout 15 "https://raw.githubusercontent.com/BrotherHua/KC-Toolbox/master/Function.sh" -o "$FUNCTION_FILE"; then
        if [ -s "$FUNCTION_FILE" ]; then
            source "$FUNCTION_FILE" && {
                echo -e "${GREEN}✓ 从备用源加载函数库成功${NC}"
                return 0
            }
        fi
    fi
