# 修改safe_download函数，增加源选择参数
safe_download() {
  local source_type=$1  # 0=GitHub, 1=Gitee
  local url=$2
  local output=$3
  local retry=3
  ...
  # 根据源类型构建完整URL
  local full_url="${BASE_URLS[$source_type]}/${url}"
  
  echo -n -e "${BLUE}正在尝试从$([ "$source_type" = "0" ] && echo "GitHub" || echo "Gitee")下载...${NC}"
  if curl -sL --connect-timeout 15 "$full_url" -o "$output"; then
    if [ -s "$output" ]; then
      echo -e "${GREEN} ✓${NC}"
      return 0
    else
...
#########
# 修改download_config函数，处理不同源的不同路径




# 修改load_functions函数（在主脚本中）
load_functions() {
    # 确保配置目录存在
    mkdir -p "${CONFIG_BASE}/KC Toolbox Script" 2>/dev/null
    
    # 尝试从GitHub下载函数库
    echo -e "\n${BLUE}正在从GitHub加载函数库...${NC}"
    if curl -sL --connect-timeout 15 "${BASE_URLS[0]}/KC's%20shell%20script/Function.sh" -o "$FUNCTION_FILE"; then
    ...
    # 如果GitHub下载失败，尝试使用Gitee源
    echo -e "${YELLOW}GitHub源下载失败，尝试Gitee源...${NC}"
    if curl -sL --connect-timeout 15 "${BASE_URLS[1]}/Function.sh" -o "$FUNCTION_FILE"; then
        if [ -s "$FUNCTION_FILE" ]; then
            source "$FUNCTION_FILE" && {
                echo -e "${GREEN}✓ 从Gitee源加载函数库成功${NC}"
                return 0
            }
        fi
    fi
...
# 常量配置
BASE_URL="https://gitee.com/BOT_141_2580/kc-config-manager/raw/master"
...
download_sensitivity() {
  ...
  case $choice in
    1)
      ...
      if safe_download "${BASE_URL}/和平精英/Active.sav" "$CONFIG_BASE/设置/Active.sav"; then
    ...
    2)
      ...
      if safe_download "${BASE_URL}/PUBGM/Active.sav" "$CONFIG_BASE/设置/GL/Active.sav"; then
...
download_config() {
  ...
  case $game in
    "和平精英")
      case $config in
        30)
          safe_download 0 "Peace%20Elite/30/Config.ini" "${CONFIG_BASE}/KC Peace Elite/Quasi/Config.ini" && \
          safe_download 0 "Peace%20Elite/30/recoil_data_kccn.ini" "${CONFIG_BASE}/KC Peace Elite/Quasi/recoil_data_kccn.ini" && \
          github_success=1
          ;;
        80)
          safe_download 0 "Peace%20Elite/80/Config.ini" "${CONFIG_BASE}/KC Peace Elite/Act/Config.ini" && \
          safe_download 0 "Peace%20Elite/80/recoil_data_kccn.ini" "${CONFIG_BASE}/KC Peace Elite/Act/recoil_data_kccn.ini" && \
          github_success=1
          ;;
      esac
      ;;
    "PUBG")
      case $config in
        30)
          safe_download 0 "PUBG/30/Config.ini" "${CONFIG_BASE}/KC PUBG/Quasi/Config.ini" && \
          safe_download 0 "PUBG/30/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/Quasi/recoil_data_kcgl.ini" && \
          github_success=1
          ;;
        80)
          safe_download 0 "PUBG/80/Config.ini" "${CONFIG_BASE}/KC PUBG/Act/Config.ini" && \
          safe_download 0 "PUBG/80/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/Act/recoil_data_kcgl.ini" && \
          github_success=1
          ;;
        "小金")
          safe_download 0 "PUBG/User/Xiao%20Jin/Config.ini" "${CONFIG_BASE}/KC PUBG/User/Xiao Jin/Config.ini" && \
          safe_download 0 "PUBG/User/Xiao%20Jin/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/User/Xiao Jin/recoil_data_kcgl.ini" && \
          github_success=1
          ;;
      esac
      ;;
  ...
  # 如果GitHub下载失败，尝试Gitee
  if [ "$github_success" -ne 1 ]; then
    echo -e "\n${YELLOW}=== GitHub下载失败，尝试Gitee ===${NC}"
    case $game in
      "和平精英")
        case $config in
          30)
            safe_download 1 "和平精英/30/Config.ini" "${CONFIG_BASE}/KC Peace Elite/Quasi/Config.ini"
            safe_download 1 "和平精英/30/recoil_data_kccn.ini" "${CONFIG_BASE}/KC Peace Elite/Quasi/recoil_data_kccn.ini"
            ;;
          80)
            safe_download 1 "和平精英/80/Config.ini" "${CONFIG_BASE}/KC Peace Elite/Act/Config.ini"
            safe_download 1 "和平精英/80/recoil_data_kccn.ini" "${CONFIG_BASE}/KC Peace Elite/Act/recoil_data_kccn.ini"
            ;;
        esac
        ;;
      "PUBG")
        case $config in
          30)
            safe_download 1 "PUBGM/30/Config.ini" "${CONFIG_BASE}/KC PUBG/Quasi/Config.ini"
            safe_download 1 "PUBGM/30/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/Quasi/recoil_data_kcgl.ini"
            ;;
          80)
            safe_download 1 "PUBGM/80/Config.ini" "${CONFIG_BASE}/KC PUBG/Act/Config.ini"
            safe_download 1 "PUBGM/80/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/Act/recoil_data_kcgl.ini"
            ;;
          "小金")
            safe_download 1 "PUBGM/用户配置/小金/Config.ini" "${CONFIG_BASE}/KC PUBG/User/Xiao Jin/Config.ini"
            safe_download 1 "PUBGM/用户配置/小金/recoil_data_kcgl.ini" "${CONFIG_BASE}/KC PUBG/User/Xiao Jin/recoil_data_kcgl.ini"
            ;;
        esac
        ;;
    esac
...
download_kc_font() {
  ...
  # 字体文件路径
  FONT_PATH="${CONFIG_BASE}/KC Toolbox Script/Amiri-Bold.ttf"
  
  # 先尝试从GitHub下载
  if ! safe_download 0 "KC's%20shell%20script/Amiri-Bold.ttf" "$FONT_PATH"; then
    # 如果GitHub失败，尝试从Gitee下载
    safe_download 1 "PUBGM/KC字体/Amiri-Bold.ttf" "$FONT_PATH"
  fi
